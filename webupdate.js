...
router.put("/user/update/edit/", async (req, res) => {
  ...
  try {
    ...
    // Check if the new Regno already exists for another user
    const checkDuplicateQuery = `SELECT Regno FROM students WHERE Regno = ?`;
    const [existing] = await connection.query(checkDuplicateQuery, [aRegno]);

    if (existing.length > 0) {
      return res.status(409).json({ success: false, message: "Reg No already exists." });
    }

    // Perform the update
    const studentUpdateQuery = `
      UPDATE students 
      SET fullname = ?, regno = ?, year = ?, sem = ?, course = ?, section = ? 
      WHERE Regno = ?`;
...
router.patch('/lectpassword/change', async (req, res) => {
  ...
  try {
      ...
      // Fetch user password
      const [rows] = await connection.query("SELECT Password FROM Lecturer WHERE Regno = ?", [Regno]);
      ...
      // Hash new password after validation
      const newHashedPassword = await bcrypt.hash(newpassword, 10);
      await connection.query("UPDATE Lecturer SET Password = ? WHERE Regno = ?", [newHashedPassword, Regno]);
