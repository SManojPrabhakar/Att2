...
router.post('/collegefee/reg/', async (req, res) => {
  ...
  try {
    ...
    const insertQuery1 = `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    await connection.query(insertQuery1, [Regno, Fullname, joiningdate, Year, Section, Course, Gender, College, Completion, hashedPassword, Role, Sem, Phone, regulation]);

    // If Fee is provided and not null, insert fee-related data
    if (Fee !== null && Fee !== undefined && !isNaN(parseFloat(Fee)) && Fee !== "") {
  const fees = parseFloat(parseFloat(Fee).toFixed(2));
  const college_fee = fees / Completion;

  // Insert into fee_register
 /* const insertQuery2 = `
    INSERT INTO fee_register (Regno, Fee_amount, Due_amount) 
    VALUES (?, ?, ?)
  `;
  await connection.query(insertQuery2, [Regno, fees, fees]);*/
  ...
  const insertFeeQuery = `
    ...
    VALUES (?, ?, ?, ?, ?)
  `;
  await connection.query(insertFeeQuery, [Regno, "College-fee", currentAcademicYear, college_fee, college_fee]);
}
    ...
    if (error.code === 'ER_DUP_ENTRY') {
      return res.status(409).json({ success: false, message: "Regno exists. Please try a different Regno." });
    }
