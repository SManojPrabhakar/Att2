...
const {course,year,sem,collegename,regulation}=req.query
  ...
  try {
      ...
      const query = `
SELECT DISTINCT s.subject_name as subjectname, s.subject_code as subjectcode
FROM subjects s
...
router.get('/results/subjects/list/', async (req, res) => {
  ...
  try {
    ...
    const query = `
      SELECT DISTINCT et.subject_name AS subjectname, et.subject_code as subjectcode
...
router.get('/student/', async (req, res) => {
  ...
  const { course, sem, year, collegename, regulation } = req.query;
  ...
  try {
    ...
    const values = [
      collegename, course, year, sem, regulation, // for regular
      collegename, course, year, sem, regulation  // for backlog
    ];
...
router.post("/upload/subjectmarks/", async (req, res) => {
  ...
  try {
    ...
    try {
      // 1. UPDATE existing external_results records
      for (const mark of marks) {
        const result_status = mark.marksObtained >= (totalMarks * 0.4) ? 'pass' : 'fail';
      ...
      if (failedStudents.length > 0) {
        ...
        const backlogQuery = `
          INSERT INTO backlog_results (
            ...
            original_year, original_sem, regulation, exam_name,
            attempt_number, marks_obtained, max_marks,
            result_status, status, added_by, added_at = NOW()
          ) VALUES ?`;
...
router.put("/marks/updated/", async (req, res) => {
  ...
  try {
    ...
    for (const markEntry of marksList) {
      const {
        regno, marks, subjectName, examname, year,
        semester, regulation, course, college_name
      } = markEntry;

      if (!regno || !subjectName || !examname || typeof marks !== 'number' ||
          !year || !semester || !regulation || !course || !college_name) {
        await connection.rollback();
        return res.status(400).json({ success: false, message: "Missing required fields in mark entry" });
      }

      const [existing] = await connection.query(
        `SELECT er.status, er.max_marks FROM external_results er
         JOIN students s ON er.Regno = s.Regno
         WHERE er.Regno = ? AND er.Subject_Name = ? AND er.exam_name = ?
         AND er.year = ? AND er.sem = ? AND er.regulation = ?
         AND s.course = ? AND s.college_name = ?`,
        [regno, subjectName, examname, year, semester, regulation, course, college_name]
      );
      ...
      if (!existing.length) {
        ...
        return res.status(404).json({
          success: false,
          message: `No record found for ${regno} - ${subjectName} - ${examname}`
        });
      }

      const { status, max_marks } = existing[0];

      if (status === 'Submitted' || status === 'Approved') {
        await connection.rollback();
        return res.status(400).json({
          success: false,
          message: `Already submitted or approved for ${regno} - ${subjectName} - ${examname}`
        });
      }
      ...
      if (resultStatus === 'fail') {
        failedStudents.push({
          regno, subjectName, subjectCode: subjectName,
          year, sem: semester, regulation, examname,
          marksObtained: marks, maxMarks: max_marks
        });
      } else {
        passedStudents.push({ regno, subjectCode: subjectName });
      }

      const [updateResult] = await connection.query(
        `UPDATE external_results er
         JOIN students s ON er.Regno = s.Regno
         SET er.marks_obtained = ?, er.result_status = ?, er.status = 'Submitted',
             er.added_by = ?, er.added_on = NOW()
         WHERE er.Regno = ? AND er.Subject_Name = ? AND er.exam_name = ?
         AND er.year = ? AND er.sem = ? AND er.regulation = ?
         AND s.course = ? AND s.college_name = ?`,
        [marks, resultStatus, decoded.Id, regno, subjectName, examname,
         year, semester, regulation, course, college_name]
      );
      ...
      if (updateResult.affectedRows === 0) {
        ...
        return res.status(400).json({
          success: false,
          message: `Failed to update marks for ${regno} - ${subjectName}`
        });
      }
    }

    // Insert failed students into backlog_results
    if (failedStudents.length > 0) {
      const [attempts] = await connection.query(
        `SELECT regno, MAX(attempt_number) AS max_attempt
         FROM backlog_results
         WHERE subject_code IN (?) AND regno IN (?)
         GROUP BY regno`,
        [
          [...new Set(failedStudents.map(s => s.subjectCode))],
          failedStudents.map(s => s.regno)
        ]
      );

      const attemptMap = new Map(attempts.map(r => [r.regno, r.max_attempt || 0]));

      const backlogValues = failedStudents.map(s => [
        s.regno, s.subjectName, s.subjectCode,
        s.year, s.sem, s.regulation, s.examname,
        (attemptMap.get(s.regno) || 0) + 1,
        s.marksObtained, s.maxMarks,
        'fail', 'submitted', decoded.Id
      ]);
    ...
    // Delete backlog records for passed students
    if (passedStudents.length > 0) {
      for (const student of passedStudents) {
        await connection.query(
          `DELETE FROM backlog_results 
           WHERE regno = ? AND subject_code = ? AND result_status = 'fail'`,
          [student.regno, student.subjectCode]
        );
      }
    }
