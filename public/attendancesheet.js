...
function getUrlValues() {
  ...
  // Assuming URL pattern is: /students/pageatt/:id/:course/:sem/:section/:year/
  const id = pathParts[3];
  const course = pathParts[4];
  const sem = pathParts[5];
  const section = pathParts[6];
  const year = pathParts[7];

  return { id, course, sem, section, year };
}
...
if (values) {
(async () => {
  const result = await attstudentslist(collegename, values.course, values.section, values.year, values.sem);
...
function createStudentCard(student, index) {
  ...
  // Toggle checkbox on card click unless clicking the checkbox or label
...
submitBtn.addEventListener('click', async () => {
  const subject_name = document.getElementById('subjectInput')?.value || 'Maths';
  const period = 1;
...
if(payload.success){
  ...
  studentCards.forEach((card) => {
   ...
   if (payload.message.includes("Unauthorized")) {
    ...
    showError(payload.message); // Show other error messages above the button
  }
}
});
