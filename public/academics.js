 ...
 const year = card.dataset.year;
  const section = card.dataset.section;
   const sem = card.dataset.sem;
    ...
    // Redirect with classId in the URL
    window.location.href = `/students/pageatt/${id}/${course}/${sem}/${section}/${year}/`;
  }
});
...
const result=await puttimetable(className,timetableArray)
 ...
 if (result.success) {
  ...
  content.innerHTML = renderList('timetables', data); // Re-render
  content.style.display = 'block';
  content.classList.add('show');
...
if (result.success) {
  ...
  content.innerHTML = renderList('subjects', data);
  content.style.display = 'block';
  content.classList.add('show');
  currentItem.querySelector('.arrow-icon').style.transform = 'rotate(180deg)';
