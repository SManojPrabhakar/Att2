...
const token = authHeader.split(' ')[1];
    ...
    try {
        ...
        const [updateResult] = await connection.query(updateQuery, [Fee, Regno, Ayear, Fee_type]);
...
const token = authHeader.split(' ')[1];
 ...
 console.log( Regno, Fee_type, Fee, Ayear, total)
...
const token = authHeader.split(' ')[1];
    ...
    const {  Fee_type, Fee, Ayear, transactionId, paymentMode, upiId } = req.body;
    let connection;
    ...
    try {
        ...
        // Step 1: Check if dues record exists
        const duesQuery = `SELECT Fee_due FROM yearly_fees WHERE Regno = ? AND Fee_year = ? AND Fee_type = ?`;
        const [duesRows] = await connection.query(duesQuery, [decoded.Regno, Ayear, Fee_type]);

        if (duesRows.length === 0) {
            return res.status(404).json({
                success: false,
                message: `No dues record found for Regno: ${decoded.Regno}, FeeType: ${Fee_type}, Academic Year: ${Ayear}`
            });
        }
        ...
        if (currentDue === 0) {
            return res.status(400).json({
                success: false,
                message: `Fee already paid for Regno: ${decoded.Regno}, FeeType: ${Fee_type}, Academic Year: ${Ayear}`
            });
        }
        ...
        const [checkResult] = await connection.query(checkQuery, [decoded.Regno, Fee_type, Ayear]);

        if (checkResult[0].count > 0) {
            throw new Error(`A payment record already exists for Regno: ${decoded.Regno}, FeeType: ${Fee_type}, Academic Year: ${Ayear}`);
        }
        ...
        const insertQuery = `
            ...
            VALUES (?, ?, ?, UNIX_TIMESTAMP(), ?, ?, ?, ?)
        `;
        await connection.query(insertQuery, [decoded.Regno, Fee_type, Fee, transactionId, paymentMode, upiId, Ayear]);

        // Step 6: Update dues
        const updateQuery = `UPDATE yearly_fees 
                             SET fee_due = fee_due - ? 
                             WHERE Regno = ? AND Fee_year = ? AND Fee_type = ?`;
        const [updateResult] = await connection.query(updateQuery, [Fee, decoded.Regno, Ayear, Fee_type]);

        if (updateResult.affectedRows === 0) {
            return res.status(404).json({
                success: false,
                message: `No matching student record found to update dues for Regno: ${decoded.Regno}, Academic Year: ${Ayear}`
            });
        }
